...
Testien rakennuksessa käytettiin `Robot Frameworkkia` sekä `Selenium-kirjastoa`. Testattava käyttöjärjestelmä rakennettiin `Reactilla`. Näiden lisäksi käytössä oli myös `Chromium driver`.
...
Selenium kirjaston kanssa kannattaa käytössä olla myös `ChromeDriver`, jonka voi ladata osoitteesta: [https://chromedriver.chromium.org/downloads](https://chromedriver.chromium.org/downloads)
...
![Kuva raportista](robotTests/images/Log_report.png)
